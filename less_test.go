...
func TestCompileUnCompressed(t *testing.T) {
	w := new(bytes.Buffer)
	lessCompiler := NewLessCompiler()
	err := lessCompiler.Compile("assets/test/signature.less", w, map[string]interface{}{"compress": false})
	...
	var expected =
`.signer-box {
  border: 1px solid #855b85;
}
.signer-box .sign-actions {
  font-size: 30px;
  width: 100%;
  text-align: center;
  display: block;
  ...
  background: rgba(133, 91, 133, 0.4);
  padding: 15px 0;
}
.signer-box .sign-actions > button.btn {
  margin: 5px;
...
.signer-box .innerbtn-main:hover {
  -webkit-box-shadow: inset 0px 0px 0px 1px #855b85;
  -moz-box-shadow: inset 0px 0px 0px 1px #855b85;
  box-shadow: inset 0px 0px 0px 1px #855b85;
}
`
	if res != expected {
